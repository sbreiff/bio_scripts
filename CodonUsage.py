...
def codonUsage(fasta_file, out_file, code = 1):
    ...
    with open(out_file, 'w') as outfile:
        for aa in 'ACDEFGHIKLMNPQRSTVWY':
            ...
            total = float(sum([codon_dict[codon] for codon in genetic_code if 
                    genetic_code[codon] == aa]))
            # create list of codons for that amino acid
            if total == 0:
                continue
            codons = [codon for codon in genetic_code if genetic_code[codon] == aa]
            for codon in codons:
                t_pct = str((codon_dict[codon]/total)*100) + '%'
                gt_pct = str((codon_dict[codon]/grandtotal)*100) + '%'
                ...
                outfile.write("%s\t%s\t%d\t%s\t%s\n" % (
                        aa, codon, codon_dict[codon], t_pct, gt_pct))
            # add blank line after each amino acid        
            outfile.write('\n')

# call function            
codonUsage(argv[1], argv[2])
